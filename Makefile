...
windows: format get
		CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -v -o kbot -ldflags "-X="github.com/balu1000/kbot.git.exe/cmd.appVersion=${VERSION}

windows/arm: format get
		CGO_ENABLED=0 GOOS=windows GOARCH=arm64 go build -v -o kbot -ldflags "-X="github.com/balu1000/kbot.git.exe/cmd.appVersion=${VERSION}
